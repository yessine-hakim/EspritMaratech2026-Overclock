...
const HomePage = () => {
    ...
    return (
        <main className="container mx-auto px-6 py-12 md:py-20" id="main-content">
            {/* Hero Section - Minimal & Left Aligned */}
            <section className="bg-primary rounded-3xl p-12 md:p-20 text-white shadow-2xl mb-20 relative border-4 border-accent/20" aria-labelledby="hero-title">
                <div className="relative z-10 max-w-3xl">
                    <h2 id="hero-title" className="text-5xl md:text-7xl font-black mb-8 leading-tight tracking-tight">Accessible Shopping for Everyone.</h2>
                    ...
                    <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-6 max-w-2xl" role="search" aria-label="Hero Search">
                        <input
                            type="text"
                            placeholder="Type product name here..."
                            className="flex-1 py-5 px-8 rounded-2xl border-2 border-white/20 bg-white/5 text-white placeholder-white/50 focus:outline-none focus:bg-white/10 focus:border-white transition-all font-bold text-xl"
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            aria-label="Search for products"
                        />
                        <button type="submit" className="px-10 py-5 bg-white text-primary rounded-2xl font-black text-xl hover:bg-accent hover:text-white transition-all shadow-xl">
                            Search Products
                        </button>
                    </form>
                </div>
            ...
            <section className="mb-24" aria-labelledby="features-title">
                <h3 id="features-title" className="text-4xl font-black text-primary mb-16 border-l-8 border-accent pl-6">Core Assistance Features</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                    <FeatureCard
                        icon={<FaCheckCircle size={32} aria-hidden="true" />}
                        title="Semantic Search"
                        desc="Find exactly what you need with intelligent product understanding using text or voice."
                    />
                    <FeatureCard
                        icon={<FaWallet size={32} aria-hidden="true" />}
                        title="Budget Awareness"
                        desc="Set your budget once, get recommendations that fit your financial profile automatically."
                    />
                    <FeatureCard
                        icon={<FaChartLine size={32} aria-hidden="true" />}
...
const FeatureCard = ({ icon, title, desc }) => (
    <div className="bg-white p-10 rounded-3xl shadow-lg border-2 border-transparent hover:border-accent/10 transition-all flex gap-8 items-start">
        <div className="flex-shrink-0 w-20 h-20 text-accent flex items-center justify-center bg-accent/5 rounded-2xl">
            {icon}
        </div>
        <div className="text-left">
            <h4 className="text-2xl font-black text-primary mb-3 leading-tight">{title}</h4>
            <p className="text-lg text-gray-medium leading-relaxed font-medium">{desc}</p>
        </div>
    </div>
);
